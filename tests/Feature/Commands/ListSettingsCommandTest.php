...
describe('list settings command', function (): void {
    test('lists stored settings rows for the requested settings class', function (): void {
        ...
        $this->artisan('settings:list', ['--settings' => ShipmentPricingSettings::class])
            ->expectsOutputToContain('shipment.pricing')
            ->assertSuccessful();
    });
});
